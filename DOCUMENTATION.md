...
```java
curr.addLine(unit whichUnit, playercolor color, string unitName, sound soundHandle, string message, TTime timeType, real timeVal, boolean wait) returns TransmissionLine
```
...
In case you don't wanna set a unit, but instead a unit-type you should enter an unit-type ID instead (default color BLACK)
    
Also you can add actions between the lines using the function:
```java
curr.addActions(real delay, TransmissionCallback cb) returns TransmissionActions
```
...
```
Where you can use the values `curr = Transmission.get()` to reffer to the actual instance, and with that use `curr.wasSkipped()`
...
to the rest of players on that force (the target force is not empty). The transmission will end only if all the implied players
skip the transmission (So if you use "All players" probably you never won't skip the transmission) but you can use the `player.getForce()` to have a force just for 1 player (Don't destroy those forces).
    
If you wanna you can also pause the transmission using:
```java
curr.pause(bool fadeOut)
// or
curr.pause(bool fadeOut, real delay) //If you wanna the transmission resume itself after a while
```
...
boolean defWillWait
```
and call `addLine()` without a parameter and set the values manually with the returned value.
